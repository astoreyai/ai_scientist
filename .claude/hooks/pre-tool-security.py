...
Input Contract (JSON from stdin):
    For PreToolUse hooks, Claude Code sends:
    {
        "tool": "Bash",           # Tool name (matched by settings.json matcher)
        "input": {                # Tool input parameters
            "command": "...",     # For Bash: the command string
            "file_path": "..."    # For Read/Write/Edit: the file path
        }
    }

    Alternative format (tool_name at root):
    {
        "name": "Bash",
        "input": {...}
    }
...
def normalize_tool_data(raw_data: Dict) -> Dict:
    """
    Normalize tool data to handle different input formats.

    Claude Code may send data in different formats:
    - {"tool": "Bash", "input": {...}}
    - {"name": "Bash", "input": {...}}
    - {"tool_name": "Bash", "tool_input": {...}}

    Args:
        raw_data: Raw input data from stdin

    Returns:
        Normalized dictionary with "name" and "input" keys
    """
    normalized = {}
    ...
    normalized["name"] = (
        ...
        raw_data.get("tool") or
        raw_data.get("tool_name") or
        "unknown"
    )

    # Extract input from various possible keys
    normalized["input"] = (
        raw_data.get("input") or
        raw_data.get("tool_input") or
        raw_data.get("parameters") or
        {}
    )
...
def main():
    ...
    try:
        ...
        # Handle empty input (some hooks may be called without data)
        if not stdin_content:
            logger.warning("No input received from stdin, approving by default")
            print(json.dumps({
                "decision": "approve",
                "reason": "No input provided - hook invoked without data",
        ...
        if not isinstance(input_data, dict):
            logger.error(f"Invalid input type: {type(input_data)}, expected dict")
            print(json.dumps({
                "decision": "approve",
                "reason": "Invalid input format (failing open)",
                "timestamp": datetime.now().isoformat()
            }))
            sys.exit(0)

        # Normalize and log
        normalized = normalize_tool_data(input_data)
        logger.info(f"Security validation triggered for tool: {normalized.get('name', 'unknown')}")
